...
def sawtooth(x, num_spikes=5):
    """
    Create a sawtooth function with num_spikes over interval [0,1]
    
    Args:
        x: Input values (should be in [0,1] for expected behavior)
        num_spikes: Number of sawtooth spikes to include
        
    Returns:
        y: Sawtooth function values
    """
    # Scale x by number of spikes and take modulo 1 to create repeating pattern
    scaled_x = (x * num_spikes) % 1.0
    
    # Create sawtooth pattern (linear ramp from 0 to 1)
    return 2.0 * scaled_x - 1.0  # Scale to [-1, 1]
...
# Add this function to create a static multi-frame summary
def create_summary_plot(splatnns, f, train_X, train_Y, mse_vals, baselines, xlim=[-1,1], Ngrid=400, fskip=10, db=True):
    ...
    for idx, frame_idx in enumerate(frame_indices):
        ...
        ax.vlines(x=np.array([s[0][0] for s in segments]), 
                 ...
                 color="k", linestyle="dashed", lw=0.5)
    ...
    for value, label in baselines:
        ...
        ax_mse.text(len(mse_vals) * 0.02, value, label, va='bottom', ha='left', 
                   fontsize=12, color=color, alpha=0.9, backgroundcolor='white',
                   bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2))
    
    # Set MSE plot settings
    ax_mse.set_xlabel('Iteration', fontsize=14)
    ax_mse.set_ylabel('log(MSE)' if db else 'MSE', fontsize=14)
...
if __name__ == "__main__": 
    ...
    # Sawtooth function parameter - number of spikes
    num_spikes = 6  # Can be adjusted to control complexity
    ...
    train_X = jr.uniform(key, (1000, 1), minval=0, maxval=1)
    

    '''
    Sin function from the paper
    y_func = lambda x: jnp.sin(20 * jnp.pi * x * (2-x))
    '''
    y_func = lambda x: sawtooth(x, num_spikes=num_spikes)

    
    train_Y = y_func(train_X)
    
    # Compute Chebyshev Monte Carlo fit MSEs
    print(f"Sawtooth function with {num_spikes} spikes")
    print('Chebyshev Monte Carlo fit MSEs:')
    cheb_k_mse = cheb_monte_carlo_fit(y_func, k)
    cheb_3k_mse = cheb_monte_carlo_fit(y_func, 3*k//2)
    
    print(f"Chebyshev MC fit log10(MSE) (k={k}): {jnp.log10(cheb_k_mse):.6f}")
    print(f"Chebyshev MC fit log10(MSE) (k={3*k//2}): {jnp.log10(cheb_3k_mse):.6f}")
    ...
    # Save animation
    output_filename = f'fit_sawtooth_{num_spikes}spikes_animation.mp4'
    R1.save(output_filename, writer='ffmpeg', fps=30)
    ...
    summary_fig.savefig(f'splat_sawtooth_{num_spikes}spikes_summary.pdf', format='pdf', bbox_inches='tight')
